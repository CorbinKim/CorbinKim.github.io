    ...
    return (
      <main className="mt mt-blog">
        <nav className="mt-nav">
          ...
          <Link className="mt-nav-back" to="/#writing">← Writing</Link>
        </nav>
    ...
    <main className="mt mt-blog">
      ...
      <article className="mt-article">
        <header className="mt-article-head">
          {post.date && <span className="mt-badge mt-badge--neutral">{formatDate(post.date)}</span>}
          <h1>{post.title}</h1>
          {post.tags.length > 0 && (
            <div className="mt-post-tags">
              {post.tags.map(t => <span key={t} className="mt-badge mt-badge--neutral">{t}</span>)}
            </div>
          )}
        </header>
